...
def start_download_thread(task_id: str, url: str, option_id: str, option_type: str, save_dir: str, browser_cookie: Optional[str] = None):
    try:
        ...
        def hook(d):
            if active_tasks.get(task_id, {}).get("_cancel_requested", False):
                raise Exception("DOWNLOAD_STOPPED_BY_USER")

            pe = active_tasks.get(task_id, {}).get("_pause_event")
            if pe and not pe.is_set():
                active_tasks[task_id]["status"] = "paused"
                active_tasks[task_id]["speed"] = "0 KB/s (Paused)"
                active_tasks[task_id]["step_message"] = "Download paused by user"
                while pe and not pe.is_set():
                    if active_tasks.get(task_id, {}).get("_cancel_requested", False):
                        raise Exception("DOWNLOAD_STOPPED_BY_USER")
                    time.sleep(0.3)
                if active_tasks.get(task_id, {}).get("status") == "paused":
...
def pause_download_task(task_id: str) -> bool:
    if task_id in active_tasks:
        pe = active_tasks[task_id].get("_pause_event")
        if pe:
            pe.clear()
            active_tasks[task_id]["status"] = "paused"
            active_tasks[task_id]["speed"] = "0 KB/s (Paused)"
...
def resume_download_task(task_id: str) -> bool:
    if task_id in active_tasks:
        ...
        if pe:
            pe.set()
            active_tasks[task_id]["status"] = "downloading"
            active_tasks[task_id]["step_message"] = "Resuming download..."
            return True
    return False

def stop_download_task(task_id: str) -> bool:
    if task_id in active_tasks:
        active_tasks[task_id]["_cancel_requested"] = True
        pe = active_tasks[task_id].get("_pause_event")
        if pe:
            pe.set()
        active_tasks[task_id]["status"] = "stopped"
        active_tasks[task_id]["step_message"] = "Stopping download..."
        return True
    return False
